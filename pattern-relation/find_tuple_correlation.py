...
# -*- coding: utf-8 -*-
'''
@Time    : 2019/3/11 11:47
@Author  : Erisu-
@contact: guoyu01988@163.com
@File    : find_tuple_correlation.py
@Software: PyCharm
@Desc:  发现相关关系
'''
...
# # my_dict中有词标和词频
# segmentor.load_with_lexicon(cws_model_path, '../data/my_dict.txt')
...
def templet_get():
    # 读取关系模板的内容
    gui = open("rule_correlation", "r", encoding='utf-8')
    for i in gui:
        i = i.strip().split()
...
def tuple_get(group, part_index, entity_index, pattern_part, file_name, sentence):
    ...
    if group:
        for t in group:
            ...
            my_r.append(t)
    for i, short_part in enumerate(my_r):
    ...
    if heads and ends:
        for head in heads:
            # 如果头实体不是在原来的尾实体组中
            if head not in old_entity:
                for end in ends:
                    if head and end:
                        if head == end:
                            continue
                        else:
                            old_entity.append(end)
...
if __name__ == '__main__':
    ...
    for classify_file in classify_path:
        ...
        if file_name in templet:
            ...
            f_three = open("three_zu/" + file_name, "a", encoding='utf-8')
            dai_chou = []
            for sentence in f_r.readlines():
                ...
                for relation_part in templet[file_name]:
                    ...
                    r = re.search(temp_pattern, sentence)
                    # 如果未成功匹配就继续
                    try:
                        t_r = r.groups()
                        if t_r:
